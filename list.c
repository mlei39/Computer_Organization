...
static User *create_user(char *name, UserType type, UserUnion data)
{
    ...
    switch (type) {
        case STUDENT:
            check = create_student(data.student.num_classes, data.student.grades, &user->data.student);
            if (check == 1){
                free(user->name);
                free(user);
                return NULL;
            ...
            break;
        case INSTRUCTOR:
            check = create_instructor(data.instructor.salary, &user->data.instructor);
            if (check == 1){
                free(user->name);
                free(user);
                return NULL;
...
static Node* create_node(char *name, UserType type, UserUnion data)
{
    Node *itself = (Node*) malloc(sizeof(Node));
    if (itself == NULL){
        return NULL;
    }

    itself->data = create_user(name, type, data);
    if (itself->data == NULL){
        free(itself);
        return NULL;
    }
...
static int user_equal(const User *user1, const User *user2)
{
    ...
    if(user1->type == STUDENT){
        if(0 == student_equal(&(user1->data.student), &(user2->data.student))){
            return 0;
        }
    }
...
int push_front(LinkedList *list, char *name, UserType type, UserUnion data)
{
    ...
    Node *newN = create_node(name, type, data);
    if (newN == NULL){
        return 1;
    }
...
int contains(LinkedList *list, User *data, User **dataOut)
{
    ...
    Node *curr = list->head;
    while(curr != NULL){
        if(1 == user_equal(curr->data,data)){
            *dataOut = curr->data;
            return 1;
        }
        curr = curr->next;
...
int num_passing_all_classes(LinkedList *list, int *dataOut)
{
    ...
    *dataOut = 0;
    for(int index = 0; index < list->size; index++){
        ...
        User *curr;
        int check = get(list, index, &curr);

        if(check == 0){
            if((curr)->type == STUDENT){
                int total = (curr)->data.student.num_classes;
                int count = 0;
                double *grade = (curr)->data.student.grades;
    ...
    for(int index = 0; index < list->size; index++){
        ...
        if(check == 0){
            if((curr)->type == INSTRUCTOR){
                num_ins += 1;
                *dataOut += (curr)->data.instructor.salary;
            }
        }
    }
    if(num_ins == 0){
