 ...
 */
 void merge(metadata_t *left, metadata_t *right) {
    left->size += right->size + sizeof(metadata_t);
 ...
 */
 metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    metadata_t *right_half = (metadata_t*) ((uint8_t*)(block + 1) + block->size - total_size);
 ...
 */
 void add_to_addr_list(metadata_t *block) {
    ...
    if (address_list == NULL){
        ...
        while (curr && ((uintptr_t) block > (uintptr_t) curr)) {
            prev = curr;
            curr = curr->next;
        }
        if(prev == NULL){
 ...
 */
 metadata_t *find_best_fit(size_t size) {
    ...
    metadata_t *curr = address_list;
    size_t min_size = 999999999999999999;

    while (curr) {
        if (curr->size == size) {
            return curr;
        }
        if (curr->size < min_size && curr->size > size){
            min_size = curr->size;
        }
        curr = curr->next;
    }
...
void *my_malloc(size_t size) {
    ...
    if(best == NULL){
        ...
        if (block == (metadata_t *) -1){
            my_malloc_errno = OUT_OF_MEMORY;
            return NULL;
        }
        // 3.c. Add the new block to the freelist. If the new block is next to another block in the freelist, merge them.
        block->size = SBRK_SIZE - sizeof(metadata_t);
        block->next = NULL;
    ...
    if (best->size > size){
        metadata_t *right = split_block(best, size);
        return right + 1;
    }
...
void *my_realloc(void *ptr, size_t size) {
    ...
    metadata_t *new_block = my_malloc(size);
    if (new_block == NULL){
        return NULL;
    }

    metadata_t *old_block = (metadata_t *)ptr;

    int min_size;
    if(old_block->size > size){
        min_size = size;
    } else {
        min_size = old_block->size;
    }
